...
public class RemoveMessageGUICallback extends AbstractGUICallback {
    ...
    public void init() {
        ...
        this.addElement(new GUITextLabel(2, 2).setDisplayText(ClientReference.localize("oxygen_mail.gui.mail.callback.removeMessage"), true, GUISettings.instance().getTitleScale()));
        this.addElement(new GUITextLabel(2, 14).setDisplayText(ClientReference.localize("oxygen_mail.gui.mail.callback.removeMessage.request"), true, GUISettings.instance().getTextScale()));
    ...
    public void handleElementClick(AbstractGUISection section, GUIBaseElement element, int mouseButton) {
        if (mouseButton == 0) { 
            if (element == this.cancelButton)
                ...
                MailManagerClient.instance().processMessageOperationSynced(this.section.getCurrentMessage().getId(), EnumMessageOperation.REMOVE_MESSAGE);
