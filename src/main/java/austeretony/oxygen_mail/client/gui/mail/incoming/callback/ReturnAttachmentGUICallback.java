...
public class ReturnAttachmentGUICallback extends AbstractGUICallback {
    ...
    public void init() {
        ...
        this.addElement(new GUITextLabel(2, 2).setDisplayText(ClientReference.localize("oxygen_mail.gui.mail.callback.returnAttachment"), true, GUISettings.instance().getTitleScale()));
        this.addElement(new GUITextLabel(2, 14).setDisplayText(ClientReference.localize("oxygen_mail.gui.mail.callback.returnAttachment.request"), true, GUISettings.instance().getTextScale()));
    ...
    public void handleElementClick(AbstractGUISection section, GUIBaseElement element, int mouseButton) {
        if (mouseButton == 0) { 
            if (element == this.cancelButton)
                ...
                MailManagerClient.instance().processMessageOperationSynced(this.section.getCurrentMessage().getId(), EnumMessageOperation.RETURN);
                this.section.sortMail(0);
                this.close();
            }
        }
    }
